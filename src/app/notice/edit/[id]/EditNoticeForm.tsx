...
export default function EditNoticeForm({ notice }: EditNoticeFormProps) {
  ...
  const [noticeDate, setNoticeDate] = useState(notice.notice_date || new Date().toISOString().split('T')[0]);
  const [sections, setSections] = useState(notice.content?.sections || [{ title: '', items: [''] }]);
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="flex justify-between items-center pt-8 border-t border-white/10">
        ...
        <button
          ...
          className="px-10 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-full font-bold transition-all shadow-lg shadow-emerald-600/20 flex items-center gap-2 disabled:opacity-50"
        >
